...
namespace SE::Net
{
	...
	public:
		bool Initialize(std::function<bool(Core::IocpObject*)> iocpObjectRegister, std::function<void(Session*)> onConnectedCallback, const char* ip, uint16_t port);
		void Close();
	...
	private:
		...
		std::function<void(Session*)> _onConnectedCallback;
	};
}
